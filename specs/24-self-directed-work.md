...
not see (stderr and journald only, unreadable by design). The daemon
supervises the agent actor and exits on its failure, so
`Restart=on-failure` stacks one entry per attempt for the next successful
boot's run to read. The gate is `(file, byte offset)` cursors plus a low token
