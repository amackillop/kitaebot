...
10. **Push** — use the `git_push` tool (never `git push` via exec). When a Review Gates section is present and the branch will become a pull request, dispatch the series gate first. **History rewriting goes through `git_fixup` and nowhere else** — never rebase, squash, or force-push via exec. For a small tweak to a commit already pushed on your branch: stage exactly the tweak, then call `git_fixup` with the target commit; it melds, verifies the tree is unchanged, and force-pushes safely. If it reports conflicts, everything is restored and the tweak is still staged — commit it normally with `git_commit` instead. If the base branch moved and a rebase is wanted, redo the PR: branch fresh from `origin/HEAD`, re-apply your work, open a new PR, and close the old one with a comment linking the replacement.
