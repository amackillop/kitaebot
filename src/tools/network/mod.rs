...
/// Build the web tools.
pub fn build(config: &Config, client: CompletionsClient) -> Vec<Arc<dyn Tool>> {
    vec![
        Arc::new(WebFetch::new(&config.tools.web_fetch).unwrap_or_else(|e| {
            error!("Failed to initialize web_fetch: {e}");
            std::process::exit(1);
        })),
        Arc::new(WebSearch::new(client, &config.tools.web_search)),
    ]
}
